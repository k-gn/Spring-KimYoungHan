...
public class QueryDslBasicTest {
	...
	/*
		...
		Spring Data JPA가 기본적으로 제공해주는 CRUD 메서드 및 쿼리 메서드를 사용하더라도,
		원하는 조건의 데이터를 가져오기 위해서 직접 쿼리를 짜야할 때가 있다.
		그럴 때 JPQL을 작성하게 되는데, 간단한 로직이라면 문제가 없으나 복잡한 로직일 경우 쿼리 문자열이 상당히 길어진다.
		게다가 오타나 문법적 오류가 있다면, 동적쿼리일 시에는 런타임 시점에 에러가 발생한다.
		문자열을 통해서 쿼리를 만들게 되면 작성한 문자열 쿼리 중 띄어쓰기 혹은 알파벳의 오류가 있을 경우 이를 컴파일 타임에서 잡아주지 못한다는 단점이 존재
		(정적 쿼리에서 오류가 있다면 어플리케이션을 로딩할 때 발견 가능하긴 함)
		...
		# 장점
			문자가 아닌 코드로 쿼리를 작성하기 때문에 컴파일 시점에 오류를 파악할 수 있다.
			자동 완성 등 IDE의 도움을 받을 수 있다.
			동적인 쿼리 작성 또한 편리하다.
			쿼리 작성시 제약 조건 등은 메서드로 추출해서 재사용할 수 있다.
			Type-Safe
			정적 쿼리라도 사용하면 좋다.
	 */
