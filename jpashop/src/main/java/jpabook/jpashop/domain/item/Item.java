...
public abstract class Item {
    ...
    /*
        ...
        트랜잭션 스크립트 패턴 : 엔티티에 로직이 거의 없고 서비스 계층에서 대부분의 로직을 처리하는 것
        도메인 주도 설계(도메인 모델 패턴) : 엔티티 자체가 해결할 수 있는 것들은 엔티티 내부에 비즈니스 로직을 넣는것이 객체지향적이다.
        데이터를 가지고 있는 곳에서 비즈니스 로직이 나가는 것이 응집도가 높다.
            밖에서 계산해서 setter 에 넣어주는식으로 하는 것이 아니라 안에서 해당 기능을 제공하면 된다.

        두 패턴이 한 프로젝트에서 양립이 가능하고, 상황에 따라 적절히 사용하면 된다.
     */
    /**
     * stock 증가
     */
